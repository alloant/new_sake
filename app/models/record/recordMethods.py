...
class RecordMethod(object):
    ...
    def get_actions(self, state, current_actor, section, panel, quick_access = False):
        ...
        if (self.flow == 'inbound' and self.stage == 'registered') or self.flow == 'internal_cr' and self.sender_id == current_actor.id and self.stage != 'shared':
            ...
            if self.flow == 'internal_cr' and panel == 'outcoming-proposals-drafts':
                actions[-1].items.append(Action(record_id=self.id,**all_actions['edit_record']))
            elif self.flow == 'internal_cr' and self.state != 'done' or self.flow == 'inbound' and (not state or state.handled == 'pending'):
    ...
    def avatar_circle(self, align, title, avatar, loop_index):
        ...
        return f'<div class="avatar-circle is-flex is-align-items-center is-justify-content-center" {style}>{avatar}</div>'
